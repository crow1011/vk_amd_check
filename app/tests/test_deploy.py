...
class ConfigTest(unittest.TestCase, ColorPrint):
    def test_conf(self):
        ...
        self.print_bold('---------------------------')
        # start server block
        self.print_info('Checking type of field server->name is string and is not empty ""')
        self.assertEqual(type(conf['server']['name']), str)
        self.assertNotEqual(len(conf['server']['name']), 0)
        self.print_pass('Done')

        self.print_info('Checking type of field server->query_interval is number and is biggest 0')
        self.assertEqual(type(conf['server']['query_interval']), int)
        self.assertEqual(conf['server']['query_interval'] > 0, True)
        self.print_pass('Done')

        self.print_info('Checking type of field server->fix_no_change is boolean')
